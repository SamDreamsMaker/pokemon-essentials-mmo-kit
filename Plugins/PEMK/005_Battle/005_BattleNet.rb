#===============================================================================
# PEMK :: BattleNet  (Phase 4c.2 — battle-stream transport + inbound queue)
#-------------------------------------------------------------------------------
...
module PEMK
  module BattleNet
    ...
    def self.send_start(to_id, props)
      PEMK.send_message({ :type => :battle_start, :from => PEMK.self_id,
    ...
    def self.send_round(to_id, round, packet)
      PEMK.send_message(packet.merge({ :type => :battle_round, :from => PEMK.self_id,
                                          :to => to_id, :round => round }))
    end

    def self.send_end(to_id, decision)
      PEMK.send_message({ :type => :battle_end, :from => PEMK.self_id,
