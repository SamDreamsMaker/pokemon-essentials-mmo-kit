#===============================================================================
# PEMK :: BattleChoiceSync  (Phase 4c.4 — canonical choice exchange)
#-------------------------------------------------------------------------------
...
module PEMK
  module BattleChoiceSync
    ...
    def pbCommandPhaseLoop(isPlayer)
      if isPlayer
        ...
        indices.each do |idx|
          ...
          PEMK::BattleNet.send_choice(@pokemmo_peer_id, @pokemmo_round, idx ^ 1, cmd)
          PEMK.log("battle: sent choice r=#{@pokemmo_round} idx=#{idx} #{cmd.inspect}")
        end
      else
        pokemmo_exchange_indices(false).each do |idx|
          ...
          PEMK.log("battle: applied remote choice r=#{@pokemmo_round} idx=#{idx} #{cmd.inspect}")
        end
      end
    end
    ...
    def pokemmo_wait_for_remote_choice(idx)
      waited = 0
      loop do
        cmd = PEMK::BattleNet.take_choice(@pokemmo_round, idx)
        return cmd if cmd
        ...
        waited += 1
        if waited > WAIT_TIMEOUT_FRAMES || !(PEMK.client && PEMK.client.connected?)
          PEMK.log("battle: remote choice timeout/disconnect (r=#{@pokemmo_round} idx=#{idx}) -> abort")
