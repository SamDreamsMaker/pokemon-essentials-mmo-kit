#===============================================================================
# PEMK :: BattleLauncher  (Phase 4b.2 / 4c.3 — start the battle)
#-------------------------------------------------------------------------------
...
module PEMK
  module BattleLauncher
    ...
    def start_pvp(remote)
      host  = PEMK::BattleNet.host?
      klass = host ? PEMK::HostBattle : PEMK::ClientBattle
      PEMK.log("battle: PvP start as #{host ? 'HOST' : 'CLIENT'}")
    ...
    def run_battle(remote, klass = Battle)
      ...
      PEMK.log("battle: starting vs #{foe_name} (#{foe_party.length} Pokemon, #{cls})")
      PEMK::BattleNet.reset   # drop any stale battle packets before this battle
      outcome = 0
      ...
      (e.backtrace || []).first(8).each { |l| PEMK.log("battle:   #{l}") }
      false
    end
  end
end
