#===============================================================================
# PokeMMO :: Inventory  (client side, Phase 2c-light)
#-------------------------------------------------------------------------------
...
class PokemonBag
  unless method_defined?(:pokemmo_orig_add)
    ...
    def add(item, qty = 1)
      ...
      PokeMMO::Inventory.notify(:bag_add, { :item => item, :qty => qty }) if ret
      ret
    end

    def remove(item, qty = 1)
      ret = pokemmo_orig_remove(item, qty)
      PokeMMO::Inventory.notify(:bag_remove, { :item => item, :qty => qty }) if ret
      ret
    end
  end
end
...
class PokemonStorage
  unless method_defined?(:pokemmo_orig_pbStoreCaught)
    ...
    def pbStoreCaught(pkmn)
      ...
      PokeMMO::Inventory.notify(:box_store, { :box => box }) if box && box >= 0
      box
    end

    def pbMove(boxDst, indexDst, boxSrc, indexSrc)
      ret = pokemmo_orig_pbMove(boxDst, indexDst, boxSrc, indexSrc)
      PokeMMO::Inventory.notify(:box_move, { :box => boxDst, :index => indexDst }) if ret
      ret
    end

    def pbDelete(box, index)
      ret = pokemmo_orig_pbDelete(box, index)
      PokeMMO::Inventory.notify(:box_delete, { :box => box, :index => index })
      ret
    end
  end
end
