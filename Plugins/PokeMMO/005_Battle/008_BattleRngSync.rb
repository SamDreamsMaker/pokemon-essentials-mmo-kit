#===============================================================================
# PokeMMO :: BattleRngSync  (Phase 4c.5 — shared authoritative RNG stream)
#-------------------------------------------------------------------------------
...
module PokeMMO
  module BattleRngSync
    def pbRandom(x = 65_536)
      if PokeMMO::BattleNet.host?
        ...
        while @pokemmo_rng_cur >= @pokemmo_rng_buf.length
          pkt = PokeMMO::BattleNet.take_round
          if pkt
    ...
    def pokemmo_flush_rng
      return unless PokeMMO::BattleNet.host?
      log  = (@pokemmo_rng_log ||= [])
      sent = (@pokemmo_rng_sent ||= 0)
      return if log.length <= sent
      PokeMMO::BattleNet.send_round(@pokemmo_peer_id, @pokemmo_round || 0,
