...
Le SDK est **un plugin** (`Plugins/PEMK/` + `meta.txt`) chargé par `PluginManager`, plus un **mode d'exécution** commuté au boot (client / serveur-headless). Le **même code Ruby MRI 3.1** tourne Windows (amis) et Linux (prod).
